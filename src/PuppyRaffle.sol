...
contract PuppyRaffle is ERC721, Ownable {
    ...
    // @audit-gas this should be immutable because it never changes
    ...
    function enterRaffle(address[] memory newPlayers) public payable {
        ...
        //q if it is an empty array do we still emit an event?
    ...
    function getActivePlayerIndex(address player) external view returns (uint256) {
        ...
        // @audit if the player is at index 0, it'll return 0 and the user might think they are not active 
        return 0;
    }
    ...
    function selectWinner() external {
        ...
        // e this is the total fees the owner should be able to connect
        // @audit overflow
        //Fixes : Newer version of solidity. Use bigger uints
        // @audit unsafe cast of unit64
        totalFees = totalFees + uint64(fee);
        
        // e when we mint a new token we use the totalSupply as the tokenId
        uint256 tokenId = totalSupply();

        // We use a different RNG calculate from the winnerIndex to determine rarity
        // @audit randomness

        // @audit people can revert the transaction till they win
        ...
        //Q
        raffleStartTime = block.timestamp;
        previousWinner = winner; // e vanity

        // q what if the winner is a smart contract with a fallback that will fail?
        // @audit the winner would nnot get the money if their fallback was messed up
